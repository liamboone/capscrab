    ...
    public GameResult GamePlay(GameConfig gc_local)
    {
        ...
        if(gc_local.isMoreBiddingLeft() == true)
        {
            ...
            for(int i=0;i<gc_local.PObjectList.size();i++)
            {
                ...
                // only allow the player to keep bidding if they have fewer than 7 letters
                if (this.isPlayerDone(i, gc_local) == false) {
                    //log.info("Requesting bid from player " + loop + " (" + currPlayer.getClass().getName() + ")");
                    bidValue = currPlayer.getBid((Letter) bidLetter.clone(),gc_local.BidList,gc_local.PlayerList,gc_local.secretstateList.get(i));
                    ...
                    log.trace("Player #" + i + " [" + currPlayer.getClass().getName() + "] no bid, has enough letters");
                }
